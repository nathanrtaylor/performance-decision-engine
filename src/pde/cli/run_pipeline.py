...
from pde.governance.audit import RunAuditor
from pde.governance.versioning import config_content_hash, resolve_active_config, resolve_raw_export_dir
from pde.ingestion.extract import load_raw_exports
from pde.ingestion.normalize import normalize_inputs
from pde.ingestion.validate import validate_inputs
...
from pde.signals.derived_metrics import synthesize_derived
from pde.scoring.assemble import assemble_scores, compute_windowed_scores
from pde.prioritization.apply import build_topic_candidates
from pde.engine.select import select_recommendations
from pde.engine.abstain import apply_abstention
from pde.engine.receipts import build_receipts
from pde.reporting.artifacts import export_run_artifacts
...
def main(argv=None) -> None:
    parser = argparse.ArgumentParser(description="Run end-to-end Performance Decision Engine pipeline.")
    parser.add_argument(
        "--raw-dir",
        type=str,
        default=None,
    ...
    if not args.no_preflight:
        from pde.governance.config_lint import lint_config, preflight_snapshot
    ...
    if scores_windowed is None or scores_windowed.empty:
        raise RuntimeError(
            "Windowed scoring returned empty. Check src/pde/temporal/aggregate.py and that "
    ...
    try:
        from pde.reporting.dashboard import write_dashboard
    ...
    try:
        from pde.reporting.expert_dashboard import write_expert_dashboard
