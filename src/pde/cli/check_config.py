...
mis-registered metric before it becomes wrong coaching.
    ...
    python -m pde.cli.check_config                         # lint active configs
    python -m pde.cli.check_config --raw-dir <snapshot>    # + raw-snapshot preflight
    python -m pde.cli.check_config --strict                # warnings fail too
...
from pde.governance.versioning import resolve_active_config, resolve_raw_export_dir
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Lint PDE config cross-references + raw snapshot")
