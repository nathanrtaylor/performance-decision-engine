...
from pde.utils.metric_format import display_map as _display_map, format_value as _format_value
from pde.reporting.dashboard_kit import (
    esc as _esc,
    fmt_int as _fmt_int,
    fmt_pct as _fmt_pct,
    fmt_num as _fmt_num,
...
def build_dashboard_html(
    ...
    parts.append(
        f"<h1>Performance Decision Engine — Run Dashboard</h1>"
    ...
    parts.append(
        f'<p class="foot">Performance Decision Engine · config {_esc(meta.get("version","-"))} · '
...
def _dampening_section(
    ...
    Dampening (src/pde/prioritization/dampening.py) flags a candidate when the same agent+topic was
...
def _page(body: str) -> str:
    return _kit_page(body, title="Performance Decision Engine — Run Dashboard", css_extra=_EXTRA_CSS)
