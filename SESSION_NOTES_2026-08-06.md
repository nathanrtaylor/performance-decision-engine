...
- `data/raw/weekly/latest/` has agents, agent_metrics, behavior_scores (+ coaching_history) — so `python -m pde.cli.run_pipeline` runs immediately off the current (late-July) snapshot.
...
4. **Wound down cleanly**: returned to `main`, deleted the 3.8 GB transcript PII pull from disk, kept the branch parked. Recorded the decision in memory (`pde-ai-root-cause-parked`).
